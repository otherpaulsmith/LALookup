function lookupPostcode() {
  ...
  fetch(`https://api.postcodes.io/postcodes/${postcode}`)
    ...
    .then(data => {
      ...
      // Step 2: Send LA code to Power Automate
      return fetch('https://prod-61.uksouth.logic.azure.com:443/workflows/f05ee7283e294d6e8702853fe8ce2bba/triggers/manual/paths/invoke?api-version=2016-06-01', {
        method: 'POST',
    ...
    .then(response => {
      if (!response.ok) throw new Error('No matching entry found in LA list');
      return response.json();
    })
    .then(laData => {
      // Step 3: Display additional info from Microsoft List
      result.innerHTML += `
        <hr>
        ...
        <strong>Website:</strong> <a href="${laData.Website}" target="_blank">${laData.Website}</a><br>
