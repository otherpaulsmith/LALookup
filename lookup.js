function lookupPostcode() {
  ...
  fetch(`https://api.postcodes.io/postcodes/${postcode}`)
    ...
    .then(data => {
      ...
      // Step 2: Call Power Automate flow with LA code
      return fetch("https://prod-61.uksouth.logic.azure.com:443/workflows/f05ee7283e294d6e8702853fe8ce2bba/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=tOJQwg90eB2sg5-cbg7eGLFgzpxQ7ruonGJIDL629kw", {
        method: "POST",
    ...
    .then(response => {
      if (!response.ok) throw new Error('No LA match found in the list');
      return response.json();
    })
    .then(laData => {
      result.innerHTML += `
        <hr>
        <p>
          <strong>DFG Email:</strong> <a href="mailto:${laData.Email}">${laData.Email || 'Not available'}</a><br>
          ...
          <strong>Adult Phone:</strong> ${laData.AdultPhone || 'Not available'}
        </p>
      `;
    })
    .catch(error => {
